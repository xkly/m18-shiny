...
shinyServer(function(input, output) {
  ...
  output$plotNamehere <- renderPlot({
    ...
    if (input$sex == "Men") {
      data.filtered <- filter(dataset, sex == "Male")
    } else if (input$sex == "Women") {
      data.filtered <- filter(dataset, sex == "Female")
    } else {
    ...
    if(input$thoughts == "Agree") {
      thoughts = data.filtered$agree
    } else {
      thoughts = data.filtered$disagree
    }
    ...
    ggplot(data.filtered, aes(x = education, y = thoughts, color = factor(sex))) + geom_point()
